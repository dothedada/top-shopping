...
describe('Cart behaviour', () => {
    ...
    it('should return null when item is not in the cart', () => {
        expect(cart.getAmount('1')).toBeNull();
    });

    it('Should return correctly the number of items', () => {
        cart.addItem('1');

        expect(cart.getAmount('2')).toBeNull();
        expect(cart.getAmount('1')).toBe(1);
    });
    ...
    it('should add 1 item correctly to the specified item', () => {
        ...
        expect(cart.getAmount('1')).toBe(3);
        expect(cart.getAmount('2')).toBe(1);
        expect(cart.getAmount('3')).toBeNull();

        cart.addItem('2');

        expect(cart.getAmount('2')).toBe(2);
    });
    ...
    it('should substract 1 item correctly to the specified item', () => {
        ...
        expect(cart.getAmount('3')).toBe(6);
    });
